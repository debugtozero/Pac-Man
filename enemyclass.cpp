...
EnemyClass::EnemyClass(GMap *map,const PlayerClass* player,int x, int y, int objecttype):BaseMoveObject(objecttype,map),Player(player),initX(x), initY(y)
{
    ...
    Frighten_Image->setScaledSize(QSize(40,40));

    QObject::connect(Frighten_Image,&QMovie::frameChanged,[=]()
    {
        setPixmap(Frighten_Image->currentPixmap());//更换当前图片为惊吓
    });
...
void EnemyClass::Move()
{
    ...
    if(Ctrl)
    {
        ...
        if (
                (((
                    flag_trace_mode ==1 //弱追踪表示只在同一行或同一列且处于警戒范围内时才追踪
                    &&
                    (
                ...
                ||
                (
                    flag_trace_mode == 2 //强追踪表示只要行与列均处于警戒范围内时就可以追踪
                    &&
...
//-------------------------Enemy1------------------------------
Enemy1::Enemy1(GMap *map,const PlayerClass* player):EnemyClass(map,player,8,8)
{
    ...
    X=8; Y=8;
    //将敌人类移动到初始实际位置
    setPos(8*BRICK_SIZE+HALF_BRICK_SIZE,8*BRICK_SIZE+HALF_BRICK_SIZE);
...
//-------------------------Enemy2------------------------------
Enemy2::Enemy2(GMap *map,const PlayerClass* player):EnemyClass(map,player,8,9)
{
    ...
    X=8; Y=9;
    setPos(9*BRICK_SIZE+HALF_BRICK_SIZE,8*BRICK_SIZE+HALF_BRICK_SIZE);
    Dir_X=Dir_Y=0; Dir=LastDir=0;
...
//-------------------------Enemy3------------------------------
Enemy3::Enemy3(GMap *map,const PlayerClass* player):EnemyClass(map,player,8,10)
{
    ...
    X=8; Y=10;
    setPos(10*BRICK_SIZE+HALF_BRICK_SIZE,8*BRICK_SIZE+HALF_BRICK_SIZE);
    Dir_X=Dir_Y=0; Dir=LastDir=0;
