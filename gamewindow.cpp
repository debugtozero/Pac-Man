    ...
    ui(new Ui::GameWindow)
{
    ...
    //设置窗口图标和标题
    this->setWindowIcon(QIcon(":/A.ico"));
    ...
    for(int i=0;i<MAPLENGTH;++i)
        for(int j=0;j<MAPLENGTH;++j)
            if(map->BeanData[i][j])
            {
                GameScene->addEllipse(0,0,6,6,QPen(Qt::white),QBrush(Qt::white))
                        ->setPos( j*BRICK_SIZE+HALF_BRICK_SIZE-6/2,i*BRICK_SIZE+HALF_BRICK_SIZE-6/2);
            }//豆子大小为6
    ...
    connect(SlowTimer, &QTimer::timeout, [=](){
        ...
        if(Player->degree_of_deceleration == 60)
            Player->SetVelocity(5-2>=0?5-2:0);
        else if(Player->degree_of_deceleration == 30)
            Player->SetVelocity(5-1>=0?5-1:0);
        else if(Player->degree_of_deceleration == 0){
            if(!Player->IsSuperTime)Player->SetVelocity(5);
            else Player->SetVelocity(5+2);
            SlowTimer->stop();
            ui->SlowDownTitle->setVisible(false);
            ui->slowProgressBar->setVisible(false);
        }
    ...
    connect(SuperTimer, &QTimer::timeout, [=](){
        ...
        {
            for(int i = 0; i < 3; i++)
                if (!Enemy[i]->HasEaten) Enemy[i]->SetVelocity(4-3>=0?4-3:0);
        }

        else if(Enemy[0]->degree_of_deceleration == 50)
        {
            for(int i = 0; i < 3; i++)
                if (!Enemy[i]->HasEaten) Enemy[i]->SetVelocity(4-2>=0?4-2:0);
        }
...
void GameWindow::MainGameFunc()
{
    ...
    //分别移动三个敌人类
    for(int i=0;i<3;++i) Enemy[i]->Move();
        ...
        for(Object=collideList.begin();Object!=collideList.end();++Object)
        {
            ...
            if((*Object)->type() == _ENEMY)
            {
                ...
                if(tmp->Frightened)
                {
                    ...
                    {
                        //加分
                        ui->ScoreValue->setText(QString::number(Scores+=200));
            ...
            else if((*Object)->type() == _SLOWVIRUS)
            {
                ...
                //减速
                Player->SetVelocity(5-3>=0?5-3:0);
            ...
            else if((*Object)->type() == _SUPERBEAN)
            {
                ...
                //加分移除
                ui->ScoreValue->setText(QString::number(Scores+=50));
            ...
            else if((*Object)->type() == _BEAN)
            {
                ...
                //增加分数并显示
                ui->ScoreValue->setText(QString::number(Scores+=20));
...
{
    int whichOne = qrand() % 10;//1~10的随机数
    int x = qrand() % MAPLENGTH;//随机生成一个0~MAPLENGTH-1的X坐标
    int y = qrand() % MAPLENGTH;//随机生成一个0~MAPLENGTH-1的Y坐标

    if(whichOne > map->PR_Bean){ //概率出现病毒 出现在空地 60%
        int Count=0;
...
bool GameWindow::eventFilter(QObject *watcher, QEvent *event)
{
    if (watcher==this)
    {
        if (event->type()==QEvent::WindowDeactivate)//if窗口失焦
        {
            Player->CleanKeyPress();//清除按键状态
            if (!Pause) PauseShift();//如果游戏没有暂停，调用函数暂停游戏
            return false;
        }
        else if (event->type()==QEvent::WindowActivate)//窗口得焦
        {
            return false;
        }
